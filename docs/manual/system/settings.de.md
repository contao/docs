...
{{% notice info %}}
Einige der Umgebungsvariablen, wie `APP_SECRET`, `DATABASE_URL` und `MAILER_DSN` ersetzen ihre jeweiligen Gegenstücke 
in der `config/parameters.yaml` und daher sollten diese Parameter nicht verwendet werden, wenn du stattdessen die Umgebungsvariable 
verwendest.
{{% /notice %}}
