...
Up to now we have worked exclusively as an administrator, who has access to all areas and elements of the system. However, a user will usually only be given access to the resources he or she actually needs for a specific task.
...
Normal users, unlike administrators, have no rights at all by default and are only allowed to do what you explicitly allow them to do. The very comprehensive permissions management in Contao allows you as an administrator to not only restrict access to certain backend modules, but also to disable every single input field if necessary.

![Activate individual input fields](/de/user-management/images/de/einzelne-eingabefelder-freischalten.png?classes=shadow)

## User groups

Each user can be a member of several user groups and automatically inherits all rights assigned to these groups. The different permissions are added together so that a member of groups A and B gets the sum of the permissions of both groups - of course only if both groups are active.

### Allowed modules

The backend navigation is created dynamically based on the user's permissions. For reasons of clarity, backend modules that have not been released will not appear in the backend navigation. Access to the theme modules can be controlled separately.

**Backend modules:** Here you define access to the backend modules.

**Theme modules:** Here you define the access to the submodules of the theme manager.
...
Similar to the page mount, which determines the entry point into the page structure, the file mount determines the entry point into the file system. The user cannot access folders outside the file mount.

![Filemounts of the user](/de/user-management/images/de/filemounts-des-benutzers.png?classes=shadow)

The user will only see the folders `files/public/media/content-images``files/public/media/documents`and`files/public/media/slider` any subfolders they may contain. All other directories, which are on the same or a higher level, are not displayed
...
**Allowed file operations:** Being able to see a directory and the files it contains does not mean a user is allowed to edit them. Here you can define what is possible with the mounted resources.
...
| Edit, copy and move files and directories | The user can rename, duplicate, and move files and directories. |
| Delete individual files and empty directories | The user may delete individual files and empty directories (not recursively). |
| Delete directories including all files and subfolders (!) | The user may delete files and directories recursively, i.e. including all contained subfolders and files. |
| Edit files in the source code editor | The user may edit the content of certain files with the source code editor directly on the server. You can define the allowed files in the backend settings. |
| Synchronize the file system with the database | The user may synchronize the file system with the database. |
...
**Allowed forms:** Here you define which forms the members of the user group are allowed to access.

**Form rights**: Here you determine whether the members of the user group may create new forms or delete existing ones.

### FAQ permissions

[FAQ extension](../../core-erweiterung/faq/)

**Allowed FAQ categories:** Here you define which FAQ categories the members of the user group can access.
...
**Distribution rights**: Here you determine whether the members of the user group may create new distribution lists or delete existing ones.
...
At the beginning of this section it was mentioned that by default normal users have no rights at all ("deny all") and you as administrator have to explicitly enable every access. This also applies to the individual input fields of each module or table listed here.

**Allowed fields:** Here you select the allowed fields.
...
**Show explanations:** By default, Contao displays a short explanation below each input field.

**Show thumbnails:** Here you can deactivate the thumbnails in the file overview of the file management, so that the directory structure loads faster.
...
**Password change necessary:**  Here you can force the user to change their password the next time they log on.
...
Users can be required to use two-factor authentication. To do this, the following configuration must be `config/config.yml`included in the
...
**Inheritance of rights:** There are the following three ways to assign rights:

| Mode | Declaration |
| ---- | ----------- |
| Use group rights only | Only the rights of the active groups are inherited. |
| Extending Group Privileges | The rights of the active groups are inherited and additionally extended by individual rights. |
...
Finally, you have to create a user and assign him or her to the group.
...
So you have to assign access rights to the pages that a user should be able to edit or create articles on and assign them either to the user or to his group. This is the prerequisite for a user to be able to click on the corresponding navigation icons.
