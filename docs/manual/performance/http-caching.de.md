...
`www.contao.org`. Die Kommunikation zwischen Client und Server findet ja bekanntlich via IP statt und ein Server kann
...
Es gibt eine Reihe an relevanten HTTP-Headern die relevant für HTTP-Caching sind. Sie alle zu erklären würde den Rahmen
...
* `miss/store`
  ...
  Der Contao-Cache konnte keinen Cache-Eintrag finden. Contao wird gestartet, generiert die Antwort und diese Antwort
  kann gecached werden und wir entsprechend abgelegt.
...
{{% expand "Heisst das, die RememberMe-Funktion deaktiviert den Cache für einen Besucher komplett?" %}}
Ja. Die Funktion von RememberMe ist, einen Besucher beim Aufruf egal welcher URL automatisch einzuloggen, sofern
...
der CookieBar kann man auch mit JavaScript dynamisch in den DOM einfügen lassen, wenn der entsprechende Key noch nicht
