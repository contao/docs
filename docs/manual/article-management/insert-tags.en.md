...
| `{{link::login}}` | This tag is replaced with a link to the login page of the current front-end user (if available). |
| `{{link_open::*}}` | Will be replaced with the opening tag of a link to an internal page: `{{link_open::12}}Hier klicken{{link_close}}`. |
| `{{link_url::*}}` | This tag is replaced with the URL of an internal page: `<a href="{{link_url::12}}">Hier klicken</a>`. |
| `{{link_target::*}}` | This tag is replaced with `target="_blank" rel="noreferrer noopener"`if the specified page is an external redirection page, and if it is set there that the link should open in a new window. |
| `{{link_title::*}}` | This tag will be replaced with the title of an internal page: `<a title="{{link_title::12}}">Hier klicken</a>`. |
...
| `{{article_open::*}}` | Is replaced with the opening tag of a link to an article: `{{article_open::12}}Hier klicken{{link_close}}`. |
...
| `{{news_url::*}}` | This tag is replaced with the URL of a message: `<a href="{{news_url::12}}">Hier klicken</a>`. |
...
| `{{event_title::*}}` | This tag will be replaced with the title of an event: `<a title="{{event_title::12}}">Hier klicken</a>`. |
| `{{calendar_feed::*}}` | This tag will be replaced with the URL to a calendar feed (replace \* with the ID) |
| `{{faq::*}}` | This tag will be replaced with a link to a frequently asked question (replace \* with the ID or alias) |
| `{{faq_open::*}}` | Replaced with the opening tag of a link to a question: `{{faq_open::12}}Hier klicken{{link_close}}`. |
...
| Insert-Tag | Description |
| ---------- | ----------- |
| `{{user::*}}` | This tag will be replaced with the content of a field of `tl_member`the registered member (replace \* with the field name). |
| `{{user::firstname}}` | This tag will be replaced with the first name of the registered member. |
| `{{user::lastname}}` | This tag is replaced with the last name of the logged in member. |
| `{{user::company}}` | This tag is replaced with the company name of the logged-in member. |
| `{{user::phone}}` | This tag will be replaced with the phone number of the registered member. |
| `{{user::mobile}}` | This tag will be replaced with the cell phone number of the registered member. |
| `{{user::fax}}` | This tag is replaced with the fax number of the registered member. |
| `{{user::email}}` | This tag is replaced with the e-mail address of the logged in member. |
...
| `{{user::country}}` | This tag is replaced with the country of the logged in member. |
...
| Insert tag | Description |
| ---------- | ----------- |
| `{{env::host}}` | This tag will be replaced with the current hostname. (for example, example.com) |
| `{{env::url}}` | This tag is replaced with the hostname and the protocol. (for example, <http://www.example.com>) |
...
| `{{insert_module::*}}` | This tag is replaced with the referenced module (replace \* with the ID of the module). |
...
| `{{news_teaser::*}}` | This tag will be replaced with the teaser of a message (replace \* with the ID of the message) |
| `{{event_teaser::*}}` | This tag will be replaced with the teaser of an event (replace \* with the ID of the event) |
| `{{file::*}}` | This tag is replaced with the contents of a .php or .html5 file from the `templates`directory (replace \* with the name). If necessary, you can pass arguments: `{{file::file.php?arg1=val}}`. You can also use UUID to get the path of a file from the database: `{{file::6939a448-9b30-11e4-bcba-079af1e9baea}}`. |
...
| `{{date::*}}` | This tag is replaced with the current date according to a custom date format. Contao supports all date and time formats that can be parsed with the [PHP function date](https://www.php.net/manual/de/function.date.php). `{{date::d.m.Y}}` |
...
| `{{last_update::*}}` | This day is replaced with the date of the last update according to a custom date format. Contao supports all date and time formats that can be parsed with the [PHP function date](https://www.php.net/manual/de/function.date.php). `{{last_update::d.m.Y}}` |
| `{{email::*}}` | This tag is replaced with an encrypted link to an email address. |
| `{{email_open::*}}` | This tag is replaced with an encrypted link to an e-mail address. However, the closing `</a>`one is not added. |
| `{{email_close}}` | This tag is replaced with `</a>`. Example: `{{email_open::foo@example.org}}E-Mail Kontakt{{email_close}}`. |
| `{{email_url::*}}` | This tag will only be replaced with the encrypted e-mail address. |
| `{{post::*}}` | With this tag a specified post variable can be read and displayed. Can be used, for example, to access individual fields of a submitted form. |
| `{{lang::*}}` | This tag can be used to mark foreign language words in a text: `{{lang::fr}}Au revoir{{lang}}`. This will be replaced with `<span lang="fr">Au revoir</span>`. |
| `{{abbr::*}}` | Mark abbreviations in a text: `{{abbr::World Wide Web}}WWW{{abbr}}`. This will be replaced with `<abbr title="World Wide Web">WWW</abbr>`. |
| `{{acronym::*}}` | Select acronyms in a text: `{{acronym::Multipurpose Internet Mail Extensions}}MIME{{acronym}}`. This is replaced with `<acronym title="Multipurpose Internet Mail Extensions">MIME</acronym>`. |
...
| `{{version}}` | This tag will be replaced with the version of Contao you are using (for example 4.8.2). |
| `{{request_token}}` | This tag will be replaced with the request token of the current session. |
| `{{toggle_view}}` | This tag is replaced with a link that switches between mobile and desktop layout. **As of Contao 4.8**, the mobile page layout is no longer part of the core distribution. If you need the feature, you have to install the package `contao/mobile-page-layout-bundle`. |
| `{{br}}` | This tag will be replaced with a HTML `<br>`element (line break). |
...
| `{{trans::*::*::*}}` | This tag can be used to output translations. Unlike the `{{label::*}}`insert tag, it can be used to output all translations from the symfony system. Example: `{{trans::MSC.updateVersion::contao_default::4.10}}`. See also the [developer documentation](https://docs.contao.org/dev/framework/translations/#accessing-translations). |
...
| `refresh` | Re-creates the output on each request. |  |
...
| `readable_size` | Converts the output to a human readable format. | see `System::getReadableSize()` |
