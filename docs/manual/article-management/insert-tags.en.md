...
| `{{form_session_data::*}}` | This tag can be used to access individual fields of a submitted form. This replaces the old `{{post::*}}` insert tag. Note that this data is only available for 10 seconds after the form submission. {{< version-tag "5.6" >}} Storing the form data in the session needs to be explicitly enabled in the form.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
The following "basic entities" are converted back into the respective HTML entities by Contao, if enabled in the DCA
via `'basicEntities' => true` in the [`eval`](https://docs.contao.org/dev/reference/dca/fields#evaluation)
definitions.
