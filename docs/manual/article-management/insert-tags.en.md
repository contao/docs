...
| `{{picture::*}}` | This tag will be replaced with a `<picture>` element and different image sizes, depending on the used image size configuration (where \* can be a database ID, a UUID or a path from the file system):<br>`{{picture::58ca4a90-2d30-11e4-8c21-0800200c9a66?size=1&template=picture_default}}`.<br>**width**: width of the preview image,<br>**height**: height of the preview image,<br>**alt**: alternative text,<br>**class**: CSS class,<br>**rel**: rel attribute (e.g. "lightbox"),<br>**mode**: mode ("proportional", "crop" or "box"),<br>**size**: ID of an image size (see Themes -&gt; Image sizes) (**size** supports the predefined image sizes from the `config.yml`, the name must be preceded by an underscore e.g. `_foo`.),<br>**template**: template to use (`picture_default`). |
...
| `{{version}}` | This tag will be replaced with the used Contao version (e.g. 4.13.2). |
...
| `absolute` | Generates an absolute path including host name and protocol |  |
