...
| `{{format_date::*::*}}`  | {{< version "4.10" >}} Mit diesem Tag kann ein UNIX Timestamp oder eine _standardisierte Datumsangabe_ formatiert werden. Der erste Parameter ist der Timestamp bzw. das Datum. Der zweite Parameter ist das Datumsformat (siehe `{{date::*}}`) und optional. Ohne Angabe des Datumsformat wird das Datum gemäß dem globalen Datumsformat ausgegeben. Dieses Tag macht in Kombination mit anderen Tags oder auch Simple Tokens Sinn, um das jeweils ausgegebene Datum bzw. ein ausgegebener Timestamp (neu) zu formatieren. <br>Beispiele: `{{date_format::{{user::tstamp}}::d.m.Y}}`, `{{date_format::##member_dateAdded##}}`. <br>Kann das Datumsformat nicht automatisch erkannt werden, kann stattdessen `{{convert_date::*::*::*}}` benutzt werden. |
| `{{convert_date::*::*::*}}` | {{< version "4.10" >}} Mit diesem Tag kann ein Datum neu formatiert werden. Der erste Parameter ist der das Datum, der zweite Parameter ist dessen Datumsformat (siehe `{{date::*}}`). Der Dritte Parameter ist das neue Datumsformat (siehe `{{date::*}}`) und optional. Ohne Angabe des Datumsformat wird das Datum gemäß dem globalen Datumsformat ausgegeben. Dieses Tag macht in Kombination mit anderen Tags oder auch Simple Tokens Sinn, um das jeweils ausgegebene Datum neu zu formatieren. <br>Beispiel: `{{convert_date::##some_date##::Y, j.n.::j. F Y}}` (konvertiert bspw. `2020, 12.7.` zu `12. Juli 2020`). |
