...
| `{{date_format::*::*}}`  | Mit diesem Tag kann eine beliebige Datumsangabe formatiert werden. Der erste Parameter ist dabei entweder ein Datum, das neu formatiert werden soll, oder ein UNIX Timestamp. Der zweite Parameter ist das Datumsformat (siehe `{{date::*}}`) und optional. Ohne Angabe des Datumsformat wird das Datum gemäß dem globalen Datumsformat ausgegeben. Dieses Tag macht in Kombination mit anderen Tags oder auch Simple Tokens Sinn, um das jeweils ausgegebene Datum bzw. ein ausgegebener Timestamp (neu) zu formatieren. Beispiele: `{{date_format::{{user::tstamp}}::d.m.Y}}`, `{{date_format::##member_dateAdded##}}` |
