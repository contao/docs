...
{{% notice note %}}
As of **Contao 4.12**, the install tool shows a warning if the database server is not running
in strict mode.
{{% /notice %}}
...
{{% notice note %}}
For MySQLi the key is named `3` instead of `1002` which is shown only if PDO is used.
{{% /notice %}}
