...
| Loupe         | `cmsig/seal-loupe-adapter`         | loupe://var/indexes/                        | Loupe runs on your local filesystem and only requires PHP and a SQLite database. The minimal requirement is that `sqlite3` (≥ 3.35.0) and `pdo_sqlite` are available in your PHP setup. |
