...
| Loupe         | `cmsig/seal-loupe-adapter`         | loupe://var/indexes/                        | Loupe läuft auf deinem lokalen Dateisystem und erfordert nur PHP und eine SQLite-Datenbank. Entsprechend ist die Minimal-Anforderung, dass `sqlite3` (≥ 3.35.0) und `pdo_sqlite` in deinem PHP-Setup zur Verfügung stehen. |
