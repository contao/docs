...
Under the hood, Contao uses its own standalone libraries [contao/image][LibraryImage] and [contao/imagine-svg][LibraryImagine]
to generate responsive images. The `CoreBundle` comes with an `ImageFactory` and `PictureFactory` that wrap around those
